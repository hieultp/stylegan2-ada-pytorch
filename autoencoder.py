...
class Mapper(pl.LightningModule):
    ...
    def step(self, batch):
        x = batch
        _, x_hat = self(x)
        loss_dict = self._calculate_loss(x_hat, x)
        return x_hat, x, loss_dict
    ...
    def on_validation_batch_end(
        ...
        if batch_idx == 0:
            ...
            images = [
                ...
                for synth, real in zip(x_hat[:n], x[:n])
            ]
            captions = ["Proj - GT"] * n
...
def train(config):
    ...
    wandb_logger = WandbLogger(
        project="nsd-bedroom256-autoencoder",
        log_model=False,
...
def main(config: DictConfig) -> None:
    log.info("Bedroom 256 autoencoder")
